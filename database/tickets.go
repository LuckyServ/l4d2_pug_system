...
func GetOpenedTicketsOfPlayer(sSteamID64 string) []DatabaseTicket {
	...
	dbQuery, errDbQuery := dbConn.Query("SELECT ticket_id,ticket_type,cteated_at FROM tickets_list WHERE created_by = '"+sSteamID64+"' AND is_closed = false ORDER BY cteated_at DESC;");
	if (errDbQuery == nil) {
...
func GetClosedTicketsOfPlayer(sSteamID64 string) []DatabaseTicket {
	MuDatabase.RLock();
	var arDBTickets []DatabaseTicket;
	dbQuery, errDbQuery := dbConn.Query("SELECT ticket_id,ticket_type,cteated_at FROM tickets_list WHERE created_by = '"+sSteamID64+"' AND is_closed = true ORDER BY cteated_at DESC LIMIT 10;");
	if (errDbQuery == nil) {
